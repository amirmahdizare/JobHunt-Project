	...
	const ShowAlertForLoginState = () => {
		...
		switch (loginState) {
			case 'error':
				showAlertContent = (
					<Alert
						style={{ width: '90%', transition: 'all 0.4s ease 0.1s' }}
						elevation={4}
						variant='filled'
						severity='error'
					>
		...
		return (
			...
			>
				<Box width={1} display='flex' justifyContent='center' mt={1}>
					{showAlertContent}
				</Box>
			</Grow>
		)
			...
			<ThemeProvider theme={() => createTheme()}>
				...
				<Box width={1} my={1} display='flex' justifyContent='space-around'>
					<Button
						...
						variant={user == 'Candidate' ? 'contained' : 'outlined'}
					>
						Candidate
					</Button>
					<Button
						...
						variant={user == 'Employer' ? 'contained' : 'outlined'}
					>
						Employer
					</Button>
				</Box>
				<FormControl
					error={
						(loginState == 'incomplete' && !values?.identifier) ||
						loginState == 'error'
					}
					fullWidth
					color='primary'
				...
				<FormControl
					error={
						(loginState == 'incomplete' && !values?.password) ||
						loginState == 'error'
					}
					fullWidth
					margin='normal'
				...
				>
					<FormControlLabel
						...
						label='Remember me'
					/>
					<Link color='secondary' href='/forgotPassword'>
						Forgot Password?
					</Link>
				</Box>
				<Box my={1} width={1}>
